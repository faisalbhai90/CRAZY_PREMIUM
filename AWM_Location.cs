...
public class AWM_Location
{
    public static async Task Run(dynamic mem, dynamic PID)
    {
        try
        {
            ...
            var result = await mem.AoBScan("20 00 00 00 69 00 6E 00 67 00 61 00 6D 00 65 00 2F 00 70 00 69 00 63 00 6B 00 75 00 70 00 2F 00 70 00 69 00 63 00 6B 00 75 00 70 00 5F 00 61 00 77 00 6D");
            ...
            if (result.Count() != 0 && result.Count() < 2)
            {
                foreach (long num in result)
                {
                    mem.WriteMemory(num.ToString("X"), "bytes",
                        "1D 00 00 00 65 00 66 00 66 00 65 00 63 00 74 00 73 00 2F 00 76 00 66 00 78 00 5F 00 69 00 6E 00 61 00 67 00 6D 00 65 00 5F 00 6C 00 61 00 73 00 65 00 72 00 5F 00 73 00 68 00 6F 00 70 00",
                        string.Empty, null);
                }
